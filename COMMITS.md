...
Правка v7: фильтр валют — **пилюли со всеми валютами** (цветная точка, hover-подсветка), без выпадающего списка; колонка «Валюта» в последних сделках читает поле `currency` (не `currencyId` — в поиске валюта лежит там); суммы в воронке подписаны символом валюты.
